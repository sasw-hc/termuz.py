...
EXPECTED_NAME = 'Pakundo.py'
if os.path.basename(__file__) != EXPECTED_NAME:
    print(f"⚠️ Uyarı: Bu dosyanın adı '{EXPECTED_NAME}' değil. Çalışmaya devam ediyorum.")
...
def login_firebase(api_key, email, password):
    try:
        ...
        response = requests.post(login_url, headers=headers, json=payload, timeout=HTTP_TIMEOUT).json()
...
BASE_URL: str = os.getenv("BASE_URL", "https://admincpm.io/Test/api")
...
def call_php_service(access_key, menu_code, token=None, email=None, password=None, extra_data=None):
    ...
    try:
        res = requests.post(url, data=payload, timeout=HTTP_TIMEOUT)
...
def send_device_os(access_key, email=None, password=None, game_label=None, telegram_id=None):
    ...
    # E-posta maskeleme: orijinal e-postayı sunucuya tam olarak göndermeyelim

    
    masked_email = (email[:2] + "***@" + email.split("@")[-1]) if (email and "@" in email) else (email or "Unknown")
    ...
    payload = {
        ...
        "email": masked_email if email is not None else "Unknown",
    ...
    try:
        response = requests.post(f"{BASE_URL}/save_device.php", json=payload, timeout=HTTP_TIMEOUT)
