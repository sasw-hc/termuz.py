...
import time
# ---- Robust HTTP session + base URL picker ----
...
def pick_working_base():
    candidates = [
        'https://admincpm.io/Test/api',
        'https://admincpm.io/test/api',
    ]
    for base in candidates:
        try:
            r = SESSION.post(f"{base}/menu.php", data={'menu': 'ping'}, timeout=12)
...
def _post_form(url, payload, timeout=20):
    try:
        ...
        if not r.ok:
            return {'ok': False, 'message': f"HTTP {info['status']} at {info['url']}. Body: {info['head']}"}
        if not r.text:
            return {'ok': False, 'message': 'Empty response from server.'}
        if 'application/json' not in info['ctype'].lower():
            return {'ok': False, 'message': f"Non-JSON response (Content-Type: {info['ctype']}) at {info['url']}. Body: {info['head']}"}
        try:
            return r.json()
        except json.JSONDecodeError as e:
            return {'ok': False, 'message': f"JSON decode error: {e}. Body: {info['head']}"}
...
# BASE_URL will be chosen dynamically
...
def call_php_service(access_key, menu_code, token=None, email=None, password=None, extra_data=None):
    ...
    # augment for add_car
    if isinstance(extra_data, dict) and menu_code.lower() == 'add_car':
        extra_data.setdefault('game_label', 'CPM2')
        if email: extra_data.setdefault('email', email)
        if token: extra_data.setdefault('token', token)
    if extra_data:
...
def send_device_os(access_key, email=None, password=None, game_label=None, telegram_id=None):
    ...
    try:
        response = SESSION.post(f\"{BASE_URL}/save_device.php\", json=payload, timeout=20)
